...
public class PayloadValidationService {
    ...
    public void validateLivekitRequest(JsonNode body) {
        ensureBody(body);
        boolean isMeeting;
        int deviceCount = body.has("devices") && body.get("devices").isArray() ? body.get("devices").size() : 0;
        if (body.has("isMeeting") && !body.get("isMeeting").isNull()) {
            isMeeting = body.get("isMeeting").asBoolean();
        } else {
            isMeeting = deviceCount > 1;
        }
